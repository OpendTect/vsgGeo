...
Copyright 2025 dGB Beheer B.V. and others.
...
#include <vsgGeo/TexturePanelStrip.h>
    ...
    , _altTileMode( 0 )
{
	...
	if ( op.getCopyFlags()==osg::CopyOp::DEEP_COPY_ALL )
	    setTexture( (vsgGeo::LayeredTexture*) node._texture->clone( op ));
	else
...
void TexturePanelStripNode::setTexture( vsgGeo::LayeredTexture* lt )
{
    if ( lt==_texture )
	return;
...
const osg::Vec2Array* TexturePanelStripNode::getCompositeTextureCoords( int geomIdx ) const
{
    ...
    const vsgGeo::Vec2i tileOrigin = _compositeCutoutOrigins[geomIdx];
    const vsgGeo::Vec2i tileSize = _compositeCutoutSizes[geomIdx];
